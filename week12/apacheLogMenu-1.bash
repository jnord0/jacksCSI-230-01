...
function displayOnlyPages(){
	cat  "$logFile" | cut -d' ' -f7 | sort | uniq
}
...
frequentVisitors() {
	cat "$logFile" | awk '{print $1, substr($4, 2, 11)}' | sort | uniq -c | awk '$1 > 10 {print $1, $2, $3}'
}
...
suspiciousVisitors(){
	grep -f "$ioc_file" "$logFile" | cut -d' ' -f1 | sort | uniq | wc -l
}
